...
class QuickSearchItem extends React.Component {
    handleNavigate = (name, mealtype_id) => {
        ...
        if (locationId) {
            this.props.history.push(`/filter?mealtype=${mealtype_id}&location=${locationId}&mealtypename=${name}&locationName=${locationName}`);
        }
        else {
            this.props.history.push(`/filter?mealtype=${mealtype_id}&mealtypename=${name}`);
        }
    }
    ...
    render() {
        ...
        return (
            <div className="col-lg-4 col-md-6 col-sm-12 block" onClick={()=> this.handleNavigate(name, mealtype_id)}>
